...
Although we work in R, Python is necessary for some operations. It is highly advised to use Pytnon >3.6. Virtual environment helps to isolate installation packages from your system, in case something goes wrong. Read [A quick-and-dirty guide on how to install packages for Python](https://snarky.ca/a-quick-and-dirty-guide-on-how-to-install-packages-for-python/) and [Lauching a Jupyter notebook from within a virtual environment]() for more information. 

Assuming you have Python3 installed, create your virtual environment.

```{sh eval=FALSE}
cd project_folder
python3.7 -m venv tensorflow # tensorflow is the name of the vitrual environment, done once
source tensorflow/bin/activate # Activate virtual environment
python -m pip install --upgrade pip # Upgrade pip installer
# Examples of commands installing deep learning-related packages
...
# python -m pip install pandas cupy cudf
python -m pip install kaggle
# Install Jupyter Lab
# python -m pip install jupyterlab
# jupyter lab # Start Jupyter Lab
deactivate # to quit tensorflow
```

```{r}
# Set up R to work with you virtual environment
dir.venv <- "/Users/mdozmorov/Documents/Work/Teaching/BIOS691_deep_learning_R/WIP/tensorflow"
# reticulate library sometimes needed to explicitly tell to use your virtual environment
# library(reticulate)
# use_virtualenv(dir.venv, required = TRUE)
```
...
2.  Install the [Kaggle API](https://github.com/Kaggle/kaggle-api) with…
      - Run `pip install kaggle` in the terminal (use virtual environment) or
...
On a Mac, some steps have to be done in command line.
...
if (!dir.exists(dogs_cats)) {
  ...
  # use `kaggle` installed in your virtual environment
  system(paste0(file.path(dir.venv, "bin", "kaggle"), " competitions download -c dogs-vs-cats"))
...
if (!dir.exists(natural_images)) {
  # use `kaggle` installed in your virtual environment
  system(paste0(file.path(dir.venv, "bin", "kaggle"), " datasets download -d prasunroy/natural-images"))
...
if (!dir.exists(amazon_data)) {
  ...
  download.file(url, destfile = "finefoods.txt.gz") # Use wget in case of an error
